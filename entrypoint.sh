...
if [ -z "${MOUNT_CHECK}" ] ; then
  ...
  CONTAINER_IP6=$(ip -j -6 route get 2001:4860:4860::8888 | jq -r '.[0] .prefsrc')

  # Reduced the information in just one line. It overwrites the default text.
  echo -e "Praqma Network MultiTool (with NGINX) - ${HOSTNAME} - ${CONTAINER_IP} - ${CONTAINER_IP6} - HTTP: ${HTTP_PORT:-80} , HTTPS: ${HTTPS_PORT:-443}" | tee  ${WEB_ROOT}/index.html 
